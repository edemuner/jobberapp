...
- **Currently implemented**: `2-notification-service` and `9-jobber-shared` have code; `1-gateway-service` now has a `src/` composition root — `config.ts`/`logger.ts` follow notification-service's pattern (`jobberConfig` singleton, `Logger` from jobber-shared), and `server.ts` exports a `GatewayServer` class with `start()` orchestrating security middleware (`cookie-session`, `hpp`, `helmet`, `cors`), standard middleware (`compression`, JSON/urlencoded body parsing), and a 404 + `CustomError`-aware error handler. Still unimplemented in gateway: routes (`routesMiddleware` is a stub), Elasticsearch health check (`startElasticSearch` is a stub), and the actual HTTP listen call (no `app.listen()`/port binding yet — `SERVER_PORT` is commented out pending that). Everything else beyond these three services is inferred intent — from `jobber-shared`'s domain interfaces (`auth`, `buyer`, `seller`, `gig`, `order`, `review`, `chat`, `search`) and the `verifyGatewayRequest` service whitelist (`auth`, `seller`, `gig`, `search`, `order`, `buyer`, `message`, `review`).
