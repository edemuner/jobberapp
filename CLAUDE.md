...
- Services follow a numbered directory convention (`2-notification-service`, `9-jobber-shared`, ...), implying slots for: gateway, authentication, users/buyer, gig, order, review, notification, and a shared library — numbers 4-8 are reserved for services not yet started.
- **Currently implemented**: `2-notification-service` and `9-jobber-shared` have code; `1-gateway-service` now has a working entrypoint — `app.ts` creates an Express app and starts a `GatewayServer` (mirrors notification-service's `app.ts`/`start()` pattern). `config.ts`/`logger.ts` follow notification-service's pattern (`jobberConfig` singleton, `Logger` from jobber-shared). `server.ts`'s `GatewayServer.start()` orchestrates security middleware (`cookie-session`, `hpp`, `helmet`, `cors`), standard middleware (`compression`, JSON/urlencoded body parsing), routing (`routesMiddleware` delegates to `appRoutes`), an Elasticsearch health check that retries `cluster.health()` until connected (`elasticsearch.ts`'s `ElasticSearch.checkConnection`), a 404 + `CustomError`-aware error handler, and binds an `http.Server` to `SERVER_PORT` (4000) via `startServer`/`startHttpServer`. Routing so far is just a `/gateway-health` GET endpoint (`controller/health.ts`'s `Health` class, mounted by `routes/health.ts`'s `HealthRoutes` and aggregated in `routes.ts`'s `appRoutes`) — no per-domain routes (auth/gig/order/etc.) exist yet. `services/auth-middleware.ts`'s `authMiddleware` (JWT-based, mirrors `jobber-shared`'s `verifyGatewayRequest` pattern but decodes the app-level session JWT rather than the gateway-to-service token) has `verifyUser`/`checkAuthentication` guards ready but not yet wired into any route. `3-auth-service` is scaffolded (package.json/tooling plus a `config.ts` `jobberConfig` singleton exposing `NODE_ENV`, `CLIENT_URL`, `JWT_TOKEN`, `GATEWAY_JWT_TOKEN`, `RABBITMQ_ENDPOINT`, `ELASTIC_SEARCH_URL`) but has no `app.ts`/`server.ts`/routes/business logic yet — dependencies installed (`bcryptjs`, `joi`, `jsonwebtoken`, `amqplib`, `express` + security middleware, `@elastic/elasticsearch`, `winston`) signal it'll issue/verify user credentials, publish auth-related RabbitMQ events, and log to Elasticsearch, but no DB client is installed yet so its datastore (docker-compose defines both a MySQL and a Postgres `jobber_auth` database, seemingly a leftover/undecided duplicate) isn't settled. Everything else beyond these four services is inferred intent — from `jobber-shared`'s domain interfaces (`auth`, `buyer`, `seller`, `gig`, `order`, `review`, `chat`, `search`) and the `verifyGatewayRequest` service whitelist (`auth`, `seller`, `gig`, `search`, `order`, `buyer`, `message`, `review`).
- **Known gap**: `9-jobber-shared`'s `verifyGatewayRequest` middleware calls `JWT.verify(token, '' as string)` — it verifies the gateway token against a hardcoded empty-string secret instead of reading a shared secret (e.g. `GATEWAY_JWT_TOKEN`) from env, so it currently performs no real signature check. Needs fixing before any service relies on it for actual auth.
