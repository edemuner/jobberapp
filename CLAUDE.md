...
- **Currently implemented**: `2-notification-service` and `9-jobber-shared` have code; `1-gateway-service` has a scaffold only (tooling/config, no `src/` yet — package.json dependencies suggest an Express gateway with JWT auth (`jsonwebtoken`, `bcrypt`, `cookie-session`), plus `cors`/`helmet`/`hpp`/`compression` middleware, `axios` for downstream HTTP calls, and `@edemuner/jobber-shared` + `@elastic/elasticsearch` for shared error/logging conventions). Everything else beyond these three is inferred intent — from `jobber-shared`'s domain interfaces (`auth`, `buyer`, `seller`, `gig`, `order`, `review`, `chat`, `search`) and the `verifyGatewayRequest` service whitelist (`auth`, `seller`, `gig`, `search`, `order`, `buyer`, `message`, `review`).
